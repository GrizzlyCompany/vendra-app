...
async function testCloseCaseFunction() {
  try {
    ...
    if (conversationsError) {
      ...
      console.log('Error details:', conversationsError);
      return;
    }
    ...
    if (closeError) {
      ...
      if (closeError.context) {
        console.log('Response status:', closeError.context.status);
        console.log('Response statusText:', closeError.context.statusText);
      }
      if (closeError.details) {
        console.log('Error details from function:', closeError.details);
      }
      
      // Try a direct fetch to get more details
      try {
        const response = await fetch(`https://vvuvuibcmvqxtvdadwne.supabase.co/functions/v1/admin-close-case`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${signInData.session.access_token}`,
            'Content-Type': 'application/json',
        ...
        console.log('Direct fetch response body:', responseBody);
      } catch (fetchError) {
        console.log('Error with direct fetch:', fetchError.message);
      }
    ...
    if (reopenError) {
      ...
      if (reopenError.context) {
        console.log('Response status:', reopenError.context.status);
        console.log('Response statusText:', reopenError.context.statusText);
      }
      if (reopenError.details) {
        console.log('Error details from function:', reopenError.details);
      }
      
      // Try a direct fetch to get more details
      try {
        const response = await fetch(`https://vvuvuibcmvqxtvdadwne.supabase.co/functions/v1/admin-reopen-case`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${signInData.session.access_token}`,
            'Content-Type': 'application/json',
    ...
    console.log('\n=== Test Complete ===');
    console.log('\nThe admin-close-case function should now work correctly in the admin panel.');
