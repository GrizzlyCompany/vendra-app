...
export const PropertyCard = memo<PropertyCardProps>(function PropertyCard({
  ...
  return (
    ...
    >
      <Card
        className={`overflow-hidden rounded-2xl shadow-lg border transition-all duration-300 hover:shadow-xl focus-within:ring-2 focus-within:ring-primary focus-within:ring-offset-2 ${className}`}
        role="article"
        aria-labelledby={`property-title-${property.id}`}
        aria-describedby={`property-description-${property.id}`}
      >
        <div className="relative aspect-video w-full overflow-hidden bg-muted">
          {/* Price badge with better accessibility */}
          <div
            className="absolute left-3 top-3 z-10 rounded-full bg-primary px-3 py-1.5 text-sm font-semibold text-primary-foreground shadow"
        ...
        <CardHeader className="pb-3">
          ...
          >
            <MapPin
              ...
              aria-hidden="true"
            />
            <span className="truncate" aria-label={`Ubicación: ${safeString(property.location, 'Ubicación no especificada')}`}>
        ...
        <CardContent className="pt-0">
          <div className="mt-4 flex flex-col sm:flex-row gap-2">
            <Button
              asChild
              className="w-full sm:flex-1 bg-primary text-primary-foreground hover:bg-primary/90 min-h-[44px] focus:ring-2 focus:ring-primary focus:ring-offset-2"
            ...
            {showEdit && (
              <Button
                asChild
                variant="outline"
                className="w-full sm:w-auto min-h-[44px] sm:w-12 sm:h-12 sm:p-0 focus:ring-2 focus:ring-primary focus:ring-offset-2"
            ...
            {onDelete && (
              <Button
                type="button"
                variant="destructive"
                className="w-full sm:w-auto min-h-[44px] sm:w-12 sm:h-12 sm:p-0 focus:ring-2 focus:ring-destructive focus:ring-offset-2"
                onClick={(e) => {
