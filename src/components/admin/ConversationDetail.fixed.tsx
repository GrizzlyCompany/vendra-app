...
interface Message {
  ...
  read_at: string | null;
  case_status: string;
  conversation_type: string; // Added this field
}
  ...
  useEffect(() => {
    ...
    const loadConversation = async () => {
      try {
        ...
        // Load messages with improved query
        console.log('Loading messages...');
        const { data: messagesData, error: messagesError } = await supabase
          .from('messages')
          .select('id, sender_id, recipient_id, content, created_at, read_at, case_status, conversation_type')
          .or(`and(sender_id.eq.${adminUser.id},recipient_id.eq.${conversationId}),and(sender_id.eq.${conversationId},recipient_id.eq.${adminUser.id})`)
          .or('conversation_type.eq.user_to_admin,conversation_type.eq.admin_to_user') // Include both types to catch misclassified messages
        ...
        const correctedMessages = (messagesData || []).map((msg: any) => {
          ...
          // Ensure all messages in this conversation are marked as user_to_admin
        ...
        console.log('Messages set, count:', correctedMessages.length);
        ...
        channel = supabase
          ...
          .on(
            ...
            (payload: any) => {
              ...
              // Correct message type if needed
              let correctedMessage = m;
              if (m.content.includes('Nuevo Reporte') && m.conversation_type !== 'user_to_admin') {
                correctedMessage = {
                  ...m,
                  conversation_type: 'user_to_admin',
                  case_status: m.case_status === 'closed' ? 'open' : m.case_status
                };
              } else if (m.conversation_type !== 'user_to_admin') {
                correctedMessage = {
                  ...m,
                  conversation_type: 'user_to_admin'
                };
              ...
              if (payload.eventType === 'INSERT') {
                setMessages((prev) => {
                  ...
                  return [...prev, correctedMessage as Message];
                });
                ...
                setMessages((prev) => {
                  return prev.map((x) => (x.id === payload.new!.id ? { ...x, ...(correctedMessage as Message) } : x));
                });
              }
            }
          )
        ...
        // Polling fallback with improved query
        poll = window.setInterval(async () => {
          try {
            ...
            // Filter and correct messages
            const correctedPollMessages = pollMessages.map((msg: any) => {
              // If message contains "Nuevo Reporte" but isn't marked as user_to_admin, correct it
              if (msg.content.includes('Nuevo Reporte') && msg.conversation_type !== 'user_to_admin') {
                return {
                  ...msg,
                  conversation_type: 'user_to_admin',
                  case_status: msg.case_status === 'closed' ? 'open' : msg.case_status
                };
              }
              // Ensure all messages in this conversation are marked as user_to_admin
              if (msg.conversation_type !== 'user_to_admin') {
            ...
            setMessages(correctedPollMessages);
            console.log('Polling updated messages, count:', correctedPollMessages.length);
          } catch (pollErr) {
  ...
  const onSend = async () => {
    ...
    try {
      const payload = {
        ...
        content: text.trim(),
        conversation_type: 'user_to_admin', // Explicitly set conversation type
        case_status: 'open' // Ensure new messages in an open case have the correct status
      };
